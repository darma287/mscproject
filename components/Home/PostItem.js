...
function PostItem({ post, onJoin, isJoined }) {
      ...
      <div className="p-5">
        ...
        <button
          ...
          className={`inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg focus:ring-4 focus:outline-none ${
            post.PlayersNeeded <= 0 || isJoined ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-700 hover:bg-blue-800 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'
          }`}
        >
          {post.PlayersNeeded <= 0 ? 'Game is full' : isJoined ? 'Joined' : 'Join'}
